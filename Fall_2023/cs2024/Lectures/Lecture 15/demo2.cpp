...
int main()
{
	...
	for (vector<string>::iterator p = stringVector.end();
		 p > stringVector.begin(); )
	{
		p--;
		cout << "Next Vector Element is: " << *p << endl; 
	}
	...
	cout << endl << "Here comes the vector in forward order (with random access off of iterator)" << endl;
	...
	cout << endl << "Here comes the vector in forward order (with random access off of vector variable)" << endl;
	for (int i=0; i<stringVector.size(); i++)
	{
		cout << "Next Vector Element is: " << stringVector[i] << endl; 
	}
	
	// This will cause an exception to be thrown as our vector does not have 23 elements!
