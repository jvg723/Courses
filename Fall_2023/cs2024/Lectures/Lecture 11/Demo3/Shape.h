//
// CS2024 -- Lecture #11, Demo 3
//
// Shape base class
//
#include <iostream>
#include <cmath>
#include <sstream> // needed for some magic below,  we haven't 
                   // gone over this yet

typedef enum {
    UNKNOWN=0,
    SQUARE,
    TRIANGLE,
    RECTANGLE,
    CIRCLE,
      ELLIPSE   // Adding new shape!
  } ShapeType;


class Shape 
{
 public:
  // Default constructor
  Shape() {
    type = UNKNOWN;
    length = 0;
    width = 0;
    radius = 0;
    base = 0;
    minor_radius = 0; // Initialize new dimension
  }

  // Set shape type and dimensions
  void setShapeType(ShapeType type) {
    this->type = type;
  }

  void setSquareDimensions(double side) {
    length = width = side;
  }

  void setRectangleDimensions(double length,double width) {
    this->length = length;
    this->width = width;
  }

  void setCircleDimensions(double radius) {
    this->radius = radius;
  }

  void setTriangleDimensions(double base, double side1, double side2) {
    this->base = base;
    // "overload" use of length and width internally for side1/side2
    length = side1;
    width = side2;
  }

  void setEllipseDimensions(double major_radius, double minor_radius) {
    // could have overloaded other existing dimensions but we
    // created a new one to illustrate the point of coding
    // this way
    this->radius = major_radius;  // don't *have* to use this-> for this one
    this->minor_radius = minor_radius;
  }


  double area() 
  {
    switch(type) {
    case SQUARE:
    case RECTANGLE:
      return length * width;
    case TRIANGLE:
      {
      // Need to use Heron's formula
      double perimeter = base + length + width;
      double p = perimeter/2.0;
      return sqrt(p*(p-base)*(p-length)*(p-width));
      }
    case CIRCLE:
      return 3.14 * radius * radius;
      // Here's our case for the new shape
    case ELLIPSE:
      return 3.14 * radius * minor_radius;
    default:
      std::cout << "I have no idea what my area is!!!" << std::endl;
      return 0;  // meaningless, we have no idea how to calculate the area!
    }
  }

  std::string dimensions()
  {
    std::ostringstream oss;

    switch(type) {
    case SQUARE:
      oss << "I'm a SQUARE.  Sides: " << length;
      return oss.str();
    case RECTANGLE:
      oss << "I'm a RECTANGLE.  Length: " << length << ", width: " << width;
      return oss.str();
    case TRIANGLE:
      oss << "I'm a TRIANGLE.  Side1: " << length << ", side2: " << width << ", base: " << base;
      return oss.str();
    case CIRCLE:
      oss << "I'm a CIRCLE.  Radius: " << radius;
      return oss.str();
      // Here's our case for the new shape
    case ELLIPSE:
      oss << "I'm an ELLIPSE.  Major Radius: " << radius << ", minor radius: " << minor_radius;
      return oss.str();
    default:
      return "UNKNOWN -- NO DIMENSIONS";
    }
  }

 private:
  // Various dimensions
  double length;
  double width;
  double base;
  double radius; 
  double minor_radius;  // declare new dimension type for ellipse
  ShapeType type;
};
