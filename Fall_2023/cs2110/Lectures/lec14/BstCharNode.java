...
class BstCharNode {
    ...
    /**
     * Return the largest value in this tree less than `bound`. Throw NoneFound if all values in
     * tree are at least as big as `bound`.
     */
    char maxBelow(char bound) throws NoneFound {
        // Compare own data to parameter to make decision
        if (data >= bound) {
            // We do not qualify; search left
            if (left == null) {
                throw new NoneFound();
            }
            return left.maxBelow(bound);
        } else {
            // We do qualify, but right subtree might have a better answer
            if (right == null) {
                return data;
            }
            try {
                return right.maxBelow(bound);
            } catch (NoneFound e) {
                return data;
            }
        }
...
/** Indicates that no items matching the specified criteria were found in a collection. */
class NoneFound extends Exception { }
